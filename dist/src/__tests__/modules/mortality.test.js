...
describe("Mortality Module", () => {
    ...
    describe("POST /api/mortalities", () => {
        it("deve registrar morte e atualizar status do animal", async () => {
            ...
            expect(prismaMock.animal.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ status: "dead" }) }));
        });
    ...
    describe("DELETE /api/mortalities/:id/photos", () => {
        it("deve remover uma foto existente", async () => {
            ...
            expect(prismaMock.mortality.update).toHaveBeenCalledWith(expect.objectContaining({ data: { photos: { set: ["url1", "url3"] } } }));
        });
...
//# sourceMappingURL=mortality.test.js.map