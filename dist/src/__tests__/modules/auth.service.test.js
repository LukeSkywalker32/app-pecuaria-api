...
describe("AuthService", () => {
    ...
    it("deve validar entrada de adminLogin e falhar para username curto", async () => {
        await expect(authService.adminLogin({ username: "ab", password: "123456" })).rejects.toThrow("Username deve ter no mínimo 3 caracteres");
    });
    it("deve validar entrada de adminLogin e falhar para senha curta", async () => {
        await expect(authService.adminLogin({ username: "admin", password: "123" })).rejects.toThrow("Senha deve ter no mínimo 6 caracteres");
    });
    it("deve falhar adminLogin quando usuário não existe", async () => {
        prismaMock.user.findFirst.mockResolvedValue(null);
        await expect(authService.adminLogin({ username: "admin", password: "123456" })).rejects.toThrow("Credenciais inválidas");
    });
    ...
    it("deve falhar login quando usuário não existe na fazenda", async () => {
        prismaMock.user.findFirst.mockResolvedValue(null);
        await expect(authService.login({ farmId: "farm-a", username: "admin", password: "123456" })).rejects.toThrow("Usuario não encontrado nesta fazenda");
    });
    it("deve falhar login quando senha estiver incorreta", async () => {
        ...
        vi.mocked(bcrypt.compare).mockResolvedValue(false);
        await expect(authService.login({ farmId: "farm-a", username: "admin", password: "123456" })).rejects.toThrow("Senha incorreta");
    });
    ...
    it("deve preservar erro de usuário inativo no renewToken", async () => {
        vi.mocked(jwt.verify).mockReturnValue({ userId: "user-1" });
    ...
    it("deve ignorar forgotPassword quando usuário não existir", async () => {
        prismaMock.user.findFirst.mockResolvedValue(null);
        await expect(authService.forgotPassword({ farmId: "farm-a", email: "x@y.com" })).resolves.toBeUndefined();
    });
    ...
    it("deve confirmar reset com sucesso", async () => {
        ...
        prismaMock.user.findUnique.mockResolvedValue({ id: "user-1", email: "x@y.com" });
        prismaMock.user.update.mockResolvedValue({});
        prismaMock.passwordResetToken.update.mockResolvedValue({});
        vi.mocked(bcrypt.hash).mockResolvedValue("hashed-new");
