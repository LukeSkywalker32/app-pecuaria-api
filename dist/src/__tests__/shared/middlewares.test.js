...
import { protectRoute, requireAnyPermission, requirePermission, } from "@/shared/middlewares/authMiddleware";
...
describe("Middlewares", () => {
    ...
    describe("authMiddleware - protectRoute", () => {
        ...
        it("deve retornar 401 se usuário não encontrado", async () => {
            ...
            const res = mockRes();
            vi.mocked(jwt.verify).mockReturnValue({ userId: "1", role: "admin" });
        ...
        it("deve carregar permissões vazias se role for desconhecida", async () => {
            const req = mockReq("Bearer valid");
            const res = mockRes();
            vi.mocked(jwt.verify).mockReturnValue({ userId: "1", role: "unknown_role" });
            prismaMock.user.findUnique.mockResolvedValue({ id: "1", active: true });
            await protectRoute(req, res, mockNext);
            expect(mockNext).toHaveBeenCalled();
        ...
        it("deve chamar next se tudo estiver correto e preencher req", async () => {
            const req = mockReq("Bearer valid");
            const res = mockRes();
            vi.mocked(jwt.verify).mockReturnValue({ userId: "1", role: "admin", farmId: "f1" });
            prismaMock.user.findUnique.mockResolvedValue({ id: "1", active: true });
    ...
    describe("authMiddleware - permissions", () => {
        ...
        it("requirePermission: sucesso se tiver permissão", () => {
            const req = { permissions: ["CREATE_USER"], role: "admin" };
        ...
        it("requirePermission: 403 se não tiver permissão", () => {
            const req = { permissions: [], role: "veterinarian" };
            const res = mockRes();
            const middleware = requirePermission("CREATE_USER");
