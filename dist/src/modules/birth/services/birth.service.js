...
function formatBirth(birth, calfAnimalId = null) {
...
class BirthService {
    ...
    async create(farmId, userId, data) {
        ...
        if (dam.status !== "active") {
            throw Object.assign(new Error(`Animal com status ${dam.status} não pode ter parto registrado`), { statusCode: 400 });
        }
     ...
     */
    async listByAnimal(farmId, damId) {
    ...
    async update(farmId, id, data) {
        ...
        if (data.birthTime !== undefined)
            updateData.birthTime = data.birthTime?.trim() ?? null;
        if (data.birthType)
            updateData.birthType = data.birthType;
        if (data.situation)
            updateData.situation = data.situation;
        if (data.deathReason !== undefined)
            updateData.deathReason = data.deathReason?.trim() ?? null;
        if (data.calfGender !== undefined)
            updateData.calfGender = data.calfGender;
        if (data.calfWeight !== undefined)
            updateData.calfWeight = data.calfWeight;
        if (data.calfEarTag !== undefined)
            updateData.calfEarTag = data.calfEarTag?.trim() ?? null;
        if (data.calfChip !== undefined)
            updateData.calfChip = data.calfChip?.trim() ?? null;
        if (data.calfStatus)
            updateData.calfStatus = data.calfStatus;
    ...
    async remove(farmId, id) {
        ...
        if (birth.calfStatus === "complete") {
            throw Object.assign(new Error("Não é possível excluir um parto cujo bezerro já foi registrado como animal. Exclua o animal primeiro."), { statusCode: 400 });
        }
