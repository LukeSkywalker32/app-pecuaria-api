...
function formatWeighing(w, gmd = null) {
...
function calculateGmd(currentWeightKg, currentDate, previousWeightKg, previousDate) {
...
async function syncAnimalCurrentWeight(farmId, animalId) {
...
async function buildGmdMap(farmId, animalId) {
...
class WeighingService {
    ...
    async list(farmId, query) {
        ...
        if (query.dateFrom || query.dateTo) {
            where.date = {};
            if (query.dateFrom)
                where.date.gte = toWeighingDate(query.dateFrom); // ✅
            if (query.dateTo)
                where.date.lte = toWeighingDate(query.dateTo); // ✅
        }
    ...
    async listByAnimal(farmId, animalId) {
        ...
        const withGmd = weighings.map(w => formatWeighing(w, gmdMap.get(w.id) ?? null));
    ...
    async update(farmId, id, data) {
        ...
        if (data.date !== undefined)
            updateData.date = toWeighingDate(data.date); // ✅
