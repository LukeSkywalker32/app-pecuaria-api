...
class EarTagService {
     ...
     */
    async markRemoved(farmId, id, data) {
     ...
     */
    async deleteRecord(farmId, id) {
        const record = await this.getById(farmId, id);
        if (record.isActive) {
            throw Object.assign(new Error("Não é possível excluir um brinco ativo. Registre a remoção antes de excluir."), { statusCode: 400 });
        }
        await prisma.earTagHistory.delete({ where: { id } });
    }
}
export default new EarTagService();
//# sourceMappingURL=earTagHisotory.service.js.map