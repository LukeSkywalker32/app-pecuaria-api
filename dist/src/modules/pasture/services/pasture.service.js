...
class PastureService {
     ...
     */
    async findById(farmId, pastureId) {
     ...
     */
    async update(farmId, pastureId, data) {
        ...
        if (data.hectares !== undefined)
            updateData.hectares = data.hectares;
        if (data.type)
            updateData.type = data.type;
        if (data.animalCapacity !== undefined)
            updateData.animalCapacity = data.animalCapacity;
     ...
     */
    async toggleActive(farmId, pastureId, active) {
        ...
        if (!active && pasture.currentAnimals > 0) {
            throw Object.assign(new Error(`Não é possível desativar um pasto com animais. Mova os ${pasture.currentAnimals} animal(is) antes.`), { statusCode: 400 });
        }
     ...
     */
    async remove(farmId, pastureId) {
        const pasture = await this.findById(farmId, pastureId);
        // Impede deletar pasto com animais
        if (pasture.currentAnimals > 0) {
            throw Object.assign(new Error(`Não é possível excluir um pasto com animais. Mova os ${pasture.currentAnimals} animal(is) antes.`), { statusCode: 400 });
        }
