...
class MortalityService {
    ...
    async update(farmId, id, data) {
        ...
        const updateData = {};
        if (data.deathDate)
            updateData.deathDate = new Date(data.deathDate);
        if (data.deathTime !== undefined)
            updateData.deathTime = data.deathTime?.trim() ?? null;
        if (data.deathLocation)
            updateData.deathLocation = data.deathLocation.trim();
        if (data.causeOfDeath)
            updateData.causeOfDeath = data.causeOfDeath.trim();
        if (data.severity !== undefined)
            updateData.severity = data.severity;
        if (data.necropsy !== undefined)
            updateData.necropsy = data.necropsy;
        if (data.disposal !== undefined)
            updateData.disposal = data.disposal?.trim() ?? null;
    ...
    async addPhotos(farmId, id, photoUrls) {
        ...
        if (total > 10) {
            throw Object.assign(new Error(`Limite de 10 fotos atingido. Já existem ${current.photos?.length ?? 0} foto(s).`), { statusCode: 400 });
        }
        const mortality = await prisma.mortality.update({
            where: { id },
            data: {
...
//# sourceMappingURL=mortality.service.js.map