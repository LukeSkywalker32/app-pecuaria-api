 ...
 */
export async function uploadToCloudinary(fileBuffer, mimeType, folder, publicId) {
    const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
    const apiKey = process.env.CLOUDINARY_API_KEY;
    const apiSecret = process.env.CLOUDINARY_API_SECRET;
    //Valida configurações antes de tentar o upload
    if (!cloudName || !apiKey || !apiSecret) {
        throw Object.assign(new Error("Cloudinary não configurado. Defina CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY e CLOUDINARY_API_SECRET"), { statusCode: 500 });
    }
    ...
    if (!response.ok) {
        ...
        throw Object.assign(new Error(`Cloudinary upload falhou: ${err.error?.message ?? response.statusText}`), { statusCode: 502 });
    }
