...
class AnimalService {
    ...
    async create(farmId, data) {
        ...
        const initialStatus = data.status || (!data.pastureId && data.origin === "purchased" ? "quarantine" : "active");
    ...
    async list(farmId, query) {
        ...
        if (query.pastureId)
            where.pastureId = query.pastureId;
        if (query.search) {
            where.OR = [
    ...
    async update(farmId, id, data) {
        ...
        if (data.sireExternalName !== undefined)
            updateData.sireExternalName = data.sireExternalName;
        if (data.sireExternalChip !== undefined)
            updateData.sireExternalChip = data.sireExternalChip;
        if (data.damExternalName !== undefined)
            updateData.damExternalName = data.damExternalName;
        if (data.damExternalChip !== undefined)
            updateData.damExternalChip = data.damExternalChip;
        ...
        const animal = await prisma.$transaction(async (tx) => {
            ...
            const isSpecialStatus = data.status === "quarantine" || data.status === "treatment";
