...
class FarmService {
    ...
    async list(callerRole, callerFarmId, query) {
    ...
    async findById(farmId, callerRole, callerFarmId) {
        // Não-Admin só ve a propria fazenda
     ...
     */
    async update(farmId, callerRole, callerFarmId, data) {
        ...
        if (data.cnpj !== undefined)
            updateData.cnpj = data.cnpj?.trim() ?? null;
     ...
     */
    async toggleActive(farmId, active) {
    ...
    async uploadLogo(farmId, callerRole, callerFarmId, fileBuffer, mimeType) {
        // Verifica acesso e existência
        await this.findById(farmId, callerRole, callerFarmId);
     ...
     */
    async uploadToCloudinary(farmId, fileBuffer, mimeType) {
        const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
        const apiKey = process.env.CLOUDINARY_API_KEY;
        const apiSecret = process.env.CLOUDINARY_API_SECRET;
        if (!cloudName || !apiKey || !apiSecret) {
            throw Object.assign(new Error("Cloudinary não está configurado. Defina a variável CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY e CLOUDINARY_API_SECRET"), { statusCode: 500 });
        }
        ...
        if (!response.ok) {
            ...
            throw Object.assign(new Error(`Cloudinary upload falhou: ${error?.error?.message ?? response.statusText}`), { statusCode: 502 });
        }
