...
class UserService {
     ...
     */
    async create(callerFarmId, data) {
        // 1. Validar campos
     ...
     */
    async update(farmId, userId, data, callerRole, callerUserId) {
        ...
        if (targetLevel >= callerLevel && callerUserId !== userId) {
            throw Object.assign(new Error("Sem permissão para editar usuário de role igual ou superior"), { statusCode: 403 });
        }
        ...
        if (data.crmv !== undefined)
            updateData.crmv = data.crmv?.trim() ?? null;
     ...
     */
    async toggleActive(farmId, userId, active) {
     ...
     */
    async changePassword(farmId, userId, data) {
     ...
     */
    async adminResetPassword(farmId, userId, data) {
     ...
     */
    async remove(farmId, userId, requestingUserId) {
        await this.findById(farmId, userId);
