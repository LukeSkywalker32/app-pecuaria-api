const VALID_ROLES = ["admin", "owner", "farmmanager", "veterinarian"];
...
function validateUpdate(data) {
    ...
    if (data.username !== undefined) {
        ...
        if (!/^[a-zA-Z0-9_.-]+$/.test(data.username)) {
            throw new Error("Nome de usuário deve conter apenas letras, números, underscores, dots e dashes");
        }
    }
...
function validateChangePassword(data) {
    ...
    if (!STRONG_PASSWORD_REGEX.test(data.newPassword)) {
        throw new Error("Nova senha deve conter maiúsculas, minúsculas, números e caracteres especiais (@$!%*?&)");
    }
