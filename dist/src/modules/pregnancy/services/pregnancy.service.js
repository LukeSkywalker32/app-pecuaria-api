...
const ATTEMPT_SELECT = {
    ...
    ultrasounds: { select: ULTRASOUND_SELECT, orderBy: { days: "asc" } },
};
const PREGNANCY_SELECT = {
    ...
    attempts: { select: ATTEMPT_SELECT, orderBy: { number: "asc" } },
};
// ─── Formatadores ───
function formatUltrasound(u) {
    return {
        id: u.id,
...
class PregnancyService {
    ...
    async create(farmId, data) {
        ...
        if (animal.status !== "active") {
            throw Object.assign(new Error(`Animal com status ${animal.status} não pode iniciar prenhez`), { statusCode: 400 });
        }
     ...
     */
    async registerAttempt(farmId, pregnancyId, data) {
        ...
        if (pregnancy.currentStatus === "pregnant") {
            throw Object.assign(new Error("Animal já está prenha — não é possível registrar nova cobertura"), { statusCode: 400 });
        }
        ...
        if (tentativaEmAndamento) {
            throw Object.assign(new Error("Já existe cobertura em andamento. Registre o ultrassom antes de nova cobertura."), { statusCode: 400 });
        }
     ...
     */
    async registerUltrasound(farmId, pregnancyId, data) {
        ...
        if (jaExiste) {
            throw Object.assign(new Error(`Ultrassom de ${data.days} dias já registrado nesta tentativa`), { statusCode: 409 });
        }
        await prisma.$transaction(async (tx) => {
