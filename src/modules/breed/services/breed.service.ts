...
const BREED_SELECT = {
   ...
   createdAt: true,
   updatedAt: true,
} satisfies Prisma.BreedSelect;

class BreedService {
   //Rota pública para o select do formulario de animais
   async list(query: ListBreedsQuery): Promise<BreedResponse[]> {
   ...
   // Busca raça por id
   async findById(id: string): Promise<BreedResponse> {
   ...
   async create(data: CreateBreedRequest): Promise<BreedResponse> {
   ...
   async update(id: string, data: UpdateBreedRequest): Promise<BreedResponse> {
      ...
      const updateData: Prisma.BreedUpdateInput = {};
   ...
   async remove(id: string): Promise<void> {
      ...
      if (animalsUsingBreed > 0) {
         throw Object.assign(
            new Error(
               `Não é possivel excluir: ${animalsUsingBreed} animal(is) cadastrado(s) com essa raça. Desative a raça ao invés de excluir.`,
            ),
            { statusCode: 409 },
         );
      }
    ...
    */
   async toggleActive(id: string, active: boolean): Promise<BreedResponse> {
      await this.findById(id);
