...
app.use((req, res, next) => {
   ...
   (req as any).farmId = "farm-id";
   (req as any).role = "admin";
   (req as any).permissions = ["place_ear_tag", "remove_ear_tag", "delete_ear_tag_history"];
   next();
});
...
describe("EarTagHistory Module", () => {
   ...
   describe("POST /api/earTagHistory", () => {
      it("deve registrar novo brinco com sucesso", async () => {
         prismaMock.animal.findFirst.mockResolvedValue(mockAnimal as any);
         ...
         prismaMock.earTagHistory.create.mockResolvedValue(mockEarTag as any);
         prismaMock.animal.update.mockResolvedValue({ ...mockAnimal, currentEarTag: "BR-12345" } as any);
      ...
      it("deve substituir brinco se já existir um ativo", async () => {
         ...
         prismaMock.earTagHistory.findFirst.mockResolvedValue(activeTag as any);
         prismaMock.earTagHistory.update.mockResolvedValue({ ...activeTag, removalDate: new Date() } as any);
         prismaMock.earTagHistory.create.mockResolvedValue(mockEarTag as any);
         prismaMock.animal.update.mockResolvedValue(mockAnimal as any);
      ...
      it("erro: animal morto", async () => {
         prismaMock.animal.findFirst.mockResolvedValue({ ...mockAnimal, status: "dead" } as any);
         const res = await request(app).post("/api/earTagHistory").send(validPlace);
         expect(res.status).toBe(400);
      });

      it("erro: animal vendido", async () => {
         prismaMock.animal.findFirst.mockResolvedValue({ ...mockAnimal, status: "sold" } as any);
         const res = await request(app).post("/api/earTagHistory").send(validPlace);
         expect(res.status).toBe(400);
      });
      ...
      it("deve cobrir animal ausente no formatRecord", async () => {
         ...
         prismaMock.earTagHistory.create.mockResolvedValue(tagNoAnimal as any);
         prismaMock.animal.update.mockResolvedValue({ ...mockAnimal, currentEarTag: "BR-12345" } as any);
   ...
   describe("GET /api/earTagHistory/animal/:animalId", () => {
      it("deve listar historico por animal", async () => {
         prismaMock.animal.findFirst.mockResolvedValue(mockAnimal as any);
         prismaMock.earTagHistory.findMany.mockResolvedValue([mockEarTag as any]);
   ...
   describe("GET /api/earTagHistory/:id", () => {
      it("deve buscar brinco por id", async () => {
         prismaMock.earTagHistory.findFirst.mockResolvedValue(mockEarTag as any);
   ...
   describe("PATCH /api/earTagHistory/:id/remove", () => {
      ...
      it("deve marcar brinco como removido", async () => {
         ...
         prismaMock.$transaction.mockImplementation(async (cb) => cb(prismaMock));
         prismaMock.earTagHistory.update.mockResolvedValue({ ...mockEarTag, removalDate: new Date() } as any);
         prismaMock.animal.findUnique.mockResolvedValue({ currentEarTag: "BR-12345" } as any);
         prismaMock.animal.update.mockResolvedValue(mockAnimal as any);
      ...
      it("deve marcar brinco sem ser currentEarTag", async () => {
         ...
         prismaMock.$transaction.mockImplementation(async (cb) => cb(prismaMock));
         prismaMock.earTagHistory.update.mockResolvedValue({ ...mockEarTag, removalDate: new Date() } as any);
         prismaMock.animal.findUnique.mockResolvedValue({ currentEarTag: "OTHER-TAG" } as any);
      ...
      it("erro: ja foi removido", async () => {
         prismaMock.earTagHistory.findFirst.mockResolvedValue({ ...mockEarTag, removalDate: new Date() } as any);
         const res = await request(app).patch("/api/earTagHistory/tag-id/remove").send(validRemove);
         expect(res.status).toBe(400);
      });
   });
   ...
   describe("DELETE /api/earTagHistory/:id", () => {
      it("deve deletar brinco inativo (já removido)", async () => {
         prismaMock.earTagHistory.findFirst.mockResolvedValue({ ...mockEarTag, removalDate: new Date() } as any);
         prismaMock.earTagHistory.delete.mockResolvedValue({} as any);
