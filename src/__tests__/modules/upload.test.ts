...
const TINY_PNG = Buffer.from(
   "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUAAdfbgnYAAAAASUVORK5CYII=",
   "base64",
);
...
describe("Upload Module", () => {
   ...
   describe("POST /api/upload/image", () => {
      ...
      it("deve propagar o statusCode quando o Cloudinary falha (ex: 502)", async () => {
         uploadToCloudinaryMock.mockRejectedValue(
            Object.assign(new Error("Cloudinary upload falhou: timeout"), { statusCode: 502 }),
         );
      ...
      it("checagem de defesa: controller rejeita size > limite mesmo se algo passar pelo multer", async () => {
         ...
         expect(fakeRes.json).toHaveBeenCalledWith(
            expect.objectContaining({ error: expect.stringContaining("Tamanho máximo permitido") }),
         );
         expect(uploadToCloudinaryMock).not.toHaveBeenCalled();
      });
   });
