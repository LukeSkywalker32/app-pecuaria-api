...
async function main() {
   ...
   console.log(
      `🔍 Buscando até ${count} animal(is) com pesagem, sem histórico antigo (>60 dias)...`,
   );
   ...
   if (candidates.length === 0) {
      console.log(
         "   Nenhum candidato encontrado — todo animal já tem pesagem antiga o suficiente.",
      );
      return;
   }
   ...
   for (const animal of candidates) {
      ...
      console.log(
         `   ✅ ${animal.name}${animal.currentEarTag ? ` (${animal.currentEarTag})` : ""} — ` +
   ...
   console.log(
      '   Pra remover depois: apague pelo Prisma Studio os registros com nota começando em "[TESTE]".',
   );
}
